...
CssLogic.prototype = {
  ...
  /**
   * Like getPropertyInfo(), but which takes an array of property names and
   * returns an array of PropertyInfos. A convenience for working with e10s.
   * @param {string[]} aProperties The CSS properties to look for.
   * @return {CssPropertyInfo[]} a array of CssPropertyInfo structures
   */
  getPropertyInfos: function CssLogic_getPropertyInfos(aProperties)
  {
    return aProperties.map(function(aProperty) {
...
CssSheet.prototype = {
  ...
  get shortSource()
  {
    ...
    let url = Cc["@mozilla.org/network/io-service;1"].
        getService(Ci["nsIIOService2"]).newURI(this.domSheet.href, null, null);
